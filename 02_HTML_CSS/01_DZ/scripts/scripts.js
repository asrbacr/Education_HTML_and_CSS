...
function fillingCardById(idEl, count, arrayEls) {
  for (let i = 0; i < count; i++) {
    ...
    clone.querySelector("a.products_card_img-button").href = arrayEls[i].hrefCard;
    clone.querySelector("p.products_card_down_header").innerHTML =
    arrayEls[i].productfillingById;
    clone.querySelector("p.products_card_down_text").innerHTML = arrayEls[i].productText;
...
// fillingCardById(tempEl, 3, productEls);

const productPageEl = document.getElementById("cardTemplateProductPage");
fillingCardById(productPageEl, 3, productEls);
